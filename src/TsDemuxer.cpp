...
TsDemuxer::TsDemuxer(bool info):m_pnum(0),m_info(info)
{
    if (m_info)
    {
        // List know PIDs in the filters.
        m_filters.insert(std::make_pair(MPEGTS_PID_NULL, DEMUXER_EVENT_NIL));
        m_filters.insert(std::make_pair(MPEGTS_PID_PAT, DEMUXER_EVENT_PAT));
    }
}
TsDemuxer::~TsDemuxer()
{
...
bool TsDemuxer::DecodePacket(const Packet& packet)
{
    ++m_pnum;
    ...
    if (f != m_filters.end()) {
        ...
        case DEMUXER_EVENT_PES:
            ...
            break;
        default:
            std::cout << "[WARNING]: PID=" << id << " filtered but not handled." << std::endl;
            break;
        }
    } else {
...
bool TsDemuxer::ReadHeader(Packet::const_iterator &p, Packet::const_iterator e, PacketHeader& header)  //read p into header
{
    if (*p != MPEGTS_SYNC_BYTE) {
        std::cerr << "[ERROR]: PKT#" << m_pnum << " Sync byte not found." << std::endl;
        return false;
    }
    p++;
...
bool TsDemuxer::ReadPAT(Packet::const_iterator &p, Packet::const_iterator e)
{
    ...
    while (p < e) {
        ...
        if (id == MPEGTS_STUFFING_BYTE) {
            if (!done) {
                std::cerr << "[ERROR]: PKT#" << m_pnum << " Empty PAT packet." << std::endl;
                return false;
            }
...
bool TsDemuxer::ReadPrograms(Packet::const_iterator &p, Packet::const_iterator e)
{
    ...
    while (p + 3 < e) {
        ...
        if (((*p&0xE0)>>5) != 0x07) {
            cerr << "[ERROR]: PKT#" << m_pnum <<" PAT reserved bits not set." << std::endl;
            return false;
        }
...
/// ////////////////////////////////////////////////////////////////////////////
/// Reads PES packet
bool TsDemuxer::ReadPES(Packet::const_iterator &p, Packet::const_iterator e, PacketHeader& header, PID id)
{
    if (header.pusi) {
        /// PES PSI
        if (p[0] == 0x0 && p[1] == 0x0 && p[2] == 0x01) {//packet_start_code_prefix
            p += 3;
            const uint8_t sidx = *p++;   //stream_id
            ...
            const uint8_t pl = *p++; //PES_header_data_length
            if (p + pl >= e) {
...
/// ////////////////////////////////////////////////////////////////////////////
void TsDemuxer::RegisterProgram(PID id, PID pid)
{
...
/// ////////////////////////////////////////////////////////////////////////////
bool TsDemuxer::RegisterStream(PID id, const Program& prog, bool video)
{
    Streams::iterator s = m_streams.find(id);
    if (s == m_streams.end()) {
        string filenamehead = (video? "video" : "audio");
        string filename = filenamehead +  "_id_" + to_string(id);
        ProgramStream *S = new ProgramStream(filename.c_str(), id, prog.id);
